<script>
	...
	onMount(async () => {
		...
		try {
			...
			error = 'Failed to generate portrait. Using fallback. Please check API key.';
			console.error('Generation error:', err);
			// Fallback already handled in generatePortrait
			const fallbackResult = await generatePortrait(data); // This will hit fallback
...
{:else if error}
	<div class="max-w-2xl mx-auto px-4 py-8">
		...
		<!-- Rest of content -->
		<h1 class="text-3xl font-bold text-gray-900 mb-4">Your Root Vagabond Portrait (Fallback)</h1>
